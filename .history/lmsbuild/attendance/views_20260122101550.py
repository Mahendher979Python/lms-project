...
def admin_attendance_create(request):
    ...
    if request.method == "POST":
        ...
        login_time_raw = request.POST.get("login_time")
        logout_time_raw = request.POST.get("logout_time")

        login_time = None
        logout_time = None

        if login_time_raw:
            login_dt = datetime.strptime(login_time_raw, "%Y-%m-%dT%H:%M")
            login_time = login_dt.time()

        if logout_time_raw:
            logout_dt = datetime.strptime(logout_time_raw, "%Y-%m-%dT%H:%M")
            logout_time = logout_dt.time()
