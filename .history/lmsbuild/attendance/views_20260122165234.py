...
def attendance_logout(request):
    ...
    try:
        ...
        start = datetime.combine(today, att.login_time)
        end = datetime.combine(today, att.logout_time)
        worked_hours = (end - start).seconds / 3600

        # 🔥 Trainer work hours rule
        if request.user.role == "trainer":
            if worked_hours < trainer_settings.daily_work_hours:
                att.status = "Half Day"
            else:
                att.status = "Present"
        else:
            # student rule already from AdminSettings
            from core_settings.models import AdminSettings
            admin_settings = AdminSettings.get()
            if worked_hours < admin_settings.half_day_hours:
                att.status = "Half Day"
            else:
                att.status = "Present"
        ...
        return JsonResponse({"error": "Attendance not found"}, status=400)
