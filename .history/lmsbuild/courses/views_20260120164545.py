...
def student_dashboard(request):
    # Example Data (Backend nunchi vastunnattu assume cheskondi)
    
    # Chart 1: Performance over months
    performance_data = [65, 78, 75, 82, 90, 85, 95] 
    months_labels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul']

    # Chart 2: Subject wise progress (Pie/Donut)
    subjects = ['Python', 'Django', 'SQL', 'JavaScript']
    subject_marks = [85, 70, 90, 60]

    context = {
        'courses_count': 4,
        # Lists ni JSON strings ga marchali JS ki pass cheyadaniki
        'performance_data': json.dumps(performance_data),
        'months_labels': json.dumps(months_labels),
        'subjects': json.dumps(subjects),
        'subject_marks': json.dumps(subject_marks),
    }
