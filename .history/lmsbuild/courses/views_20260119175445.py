...
def admin_course_add(request):
    if request.method == "POST":
        Course.objects.create(
            ...
            is_published=request.POST.get("status") == "published"
        )
        return redirect("admin_course_list")

    return render(request, "courses/admin/course_add.html")
...
def admin_course_edit(request, course_id):
    ...
    - trainer
    - thumbnail / video / pdf (optional replace)
    ...
    if request.method == "POST":
        ...
        if "video" in request.FILES:
            course.intro_video = request.FILES["video"]

        if "pdf" in request.FILES:
            course.syllabus_pdf = request.FILES["pdf"]
