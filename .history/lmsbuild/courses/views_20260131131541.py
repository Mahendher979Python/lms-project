...
def trainer_material_list(request, course_id):
    course = get_object_or_404(Course, id=course_id, trainer=request.user.teacherprofile)
    materials = CourseMaterial.objects.filter(course=course)
...
@login_required
def trainer_material_edit(request, material_id):

    # only allow trainers
    if not hasattr(request.user, "teacher_profile"):
        return redirect("admin_material_list")

    material = get_object_or_404(
        CourseMaterial,
        id=material_id,
        course__trainer=request.user.teacher_profile
    )
    ...
    if request.method == "POST":
        ...
        return redirect("trainer_material_list", course_id=material.course.id)
...
def trainer_course_list(request):
    ...
    courses = Course.objects.filter(trainer=request.user.teacher_profile)
