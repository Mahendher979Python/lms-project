...
def student_course_list(request):
    ...
    # FINAL courses = trainer + enrollment match
...
def student_material_redirect(request):
    ...
    if not enrollment:
        messages.error(request, "No enrolled courses")
        return redirect("student_dashboard")

    return redirect("student_material_list", course_id=enrollment.course.id)
...
def student_material_list(request, course_id):
    ...
    # MUST be enrolled + trainer match
    enrollment = Enrollment.objects.filter(
        student=request.user,
        course_id=course_id,
        is_active=True,
        course__trainer=student.trainer
    ).first()

    if not enrollment:
        messages.error(request,"Access denied")
        return redirect("student_material_redirect")
...
@login_required
def student_material_preview(request, pk):
    ...
    return render(request,"courses/material/student/preview.html",{
        "material": material
    })
