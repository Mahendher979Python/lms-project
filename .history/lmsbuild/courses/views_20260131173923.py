...
def trainer_courses(request):
    ...
    return render(request,
        "courses/trainer_courses/course_list.html",
        {"courses": courses}
    )
...
def trainer_material_upload(request,course_id):
    ...
    return render(request,"material/upload.html",{"course":course})

@login_required
def trainer_material_edit(request,pk):

    material = get_object_or_404(CourseMaterial,id=pk,course__trainer=request.user)
    ...
    if request.method=="POST":
        ...
        return redirect("trainer_materials",material.course.id)
