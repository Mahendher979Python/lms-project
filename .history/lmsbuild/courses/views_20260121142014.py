...
def student_course_detail(request, course_id):
    ...
    topics = Lesson.objects.filter(course=course).order_by("order")

    selected_topic = None
    topic_id = request.GET.get("topic")

    if topic_id:
        selected_topic = get_object_or_404(Lesson, id=topic_id, course=course)

    return render(
        request,
        "courses/student/course_folder.html",
        {
            "course": course,
            "topics": topics,
