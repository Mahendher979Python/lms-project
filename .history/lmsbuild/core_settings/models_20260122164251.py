...
class TrainerSettings(models.Model):
    ...
    approval_required = models.BooleanField(
        default=True,
        help_text="Admin approval required for trainer account"
    )
    profile_editable = models.BooleanField(
        default=True,
        help_text="Trainer can edit own profile"
    )
    ...
    can_create_courses = models.BooleanField(
        default=True,
        help_text="Trainer allowed to create courses"
    )
    course_approval_required = models.BooleanField(
        default=True,
        help_text="Admin approval required before course publish"
    )
    max_courses = models.IntegerField(
        default=5,
        help_text="Maximum courses trainer can create"
    )
    ...
    attendance_mandatory = models.BooleanField(
        default=True,
        help_text="Trainer attendance mandatory"
    )
    daily_work_hours = models.FloatField(
        default=8.0,
        help_text="Required daily work hours"
    )
    late_login_minutes = models.IntegerField(
        default=15,
        help_text="Allowed late login time in minutes"
    )
    auto_mark_absent = models.BooleanField(
        default=True,
        help_text="Auto mark absent if no login"
    )
    attendance_approval_required = models.BooleanField(
        default=True,
        help_text="Admin approval required for trainer attendance"
    )
