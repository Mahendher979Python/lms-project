...
def trainer_notes_list(request):
    ...
    notes = StudyItem.objects.filter(
        ...
        Q(created_by=request.user) |      # trainer's own notes
        Q(assigned_to=request.user)       # admin assigned notes
    ).distinct().order_by("-created_at")

    return render(
        request,
        "study/trainer/notes/notes_list.html",
        {"notes": notes}
    )
...
def trainer_notes_add(request):
    ...
    return render(
        request,
        "study/trainer/notes/notes_add.html",
        {"students": students},
    )
