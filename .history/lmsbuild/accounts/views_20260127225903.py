...
def admin_user_list(request):
    ...
    for t in trainers:
        students = Student.objects.filter(trainer=t).select_related("batch")

        total_students = students.count()

        # unique batch names
        batches = students.values_list("batch__name", flat=True).distinct()

        data.append({
            "trainer": t,
            "total_students": total_students,
            "batches": ", ".join([b for b in batches if b])
        })

    return render(request, "accounts/admin/user/admin_user_list.html", {
        "data": data
    })
...
@admin_required
def admin_trainer_detail(request, id):

    trainer = get_object_or_404(TeacherProfile, user__id=id)

    students = Student.objects.filter(trainer=trainer).select_related("batch")
    ...
    return render(request, "accounts/admin/trainers/detail.html", {
        "trainer": trainer,
        "students": students,
        "total_students": total_students,
