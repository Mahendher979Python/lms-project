...
def admin_add_student(request):
    ...
    if request.method == "POST":
        ...
        username = request.POST.get("username", "").strip()
        trainer_user_id = request.POST.get("trainer")
        ...
        try:
            trainer_user = get_object_or_404(User, id=trainer_user_id, role="trainer")
            trainer_profile = get_object_or_404(TeacherProfile, user=trainer_user)
...
def admin_edit_student(request, id):
    ...
    if request.method == "POST":
        ...
        # Trainer change
        trainer_user = get_object_or_404(User, id=request.POST.get("trainer"), role="trainer")
        student.trainer = get_object_or_404(TeacherProfile, user=trainer_user)

        # Batch change
        student.batch = get_object_or_404(Batch, id=request.POST.get("batch"))
