...
def admin_add_student(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        student.user = user
        student.trainer = trainer   # 🔥 MAIN LINK
        student.save()

        messages.success(request, "Student added under trainer successfully")
...
def admin_edit_student(request, id):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        trainer_id = request.POST.get("trainer")
        if trainer_id:
            student.trainer = get_object_or_404(TeacherProfile, id=trainer_id)
