...
def admin_batch_list(request):
    batches = Batch.objects.all()
    return render(request, "admin/batches/list.html", {"batches": batches})
...
def admin_batch_create(request):
    ...
    return render(request, "admin/batches/create.html", {"teachers": teachers})
...
def admin_batch_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        trainer_id = request.POST.get("trainer")
        batch.trainer = TeacherProfile.objects.get(id=trainer_id)
        batch.save()

        return redirect("admin_batch_list")

    return render(request, "admin/batches/edit.html", {
        "batch": batch,
        "teachers": teachers
    })
