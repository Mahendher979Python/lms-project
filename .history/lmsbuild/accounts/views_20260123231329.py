...
from .models import Student, TeacherProfile  # Ensure these models exist
from .forms import StudentCreateForm, StudentUpdateForm # Ensure these forms exist

# --- Custom Decorator for Admin Role Check ---
def admin_required(view_func):
    def wrapper_func(request, *args, **kwargs):
        if request.user.is_authenticated and request.user.role == 'admin': # Adjust 'role' based on your user model
            return view_func(request, *args, **kwargs)
        return redirect('login')
    return wrapper_func
...
def admin_students(request):
    # Select related optimizes the query by fetching user and trainer details in one go
...
def admin_add_student(request):
    ...
    if request.method == "POST":
        # Note: We validate form manually here or via form.is_valid()
        # Assuming you handle the User creation manually as shown in your snippet:
        
        username = request.POST.get("username")
        email = request.POST.get("email")
        trainer_id = request.POST.get("trainer")

        if username and email and trainer_id:
            ...
            user = User.objects.create_user(
                username=username,
                email=email,
                password=get_random_string(8), # Generates random password
                role="student", # Ensure your User model supports 'role' field
                is_active=True
            )
...
def admin_delete_student(request, id):
    student = get_object_or_404(Student, id=id)
    student.user.is_active = False # Soft delete
    student.user.save()
